...
export default async function handler(req, res) {
  ...
  try {
    ...
    jsonScripts.forEach(m => {
      try {
        allJsonObjects.push(JSON.parse(m[1].trim()));
      } catch (e) {}
    });

    // Extract Niobe data if present
    const niobeMatch = html.match(/niobeClientData\":([\s\S]*?\]\])/);
    if (niobeMatch) {
      try {
        allJsonObjects.push(JSON.parse(niobeMatch[1]));
      } catch (e) {}
    }
    ...
    if (nextDataMatch) {
      try {
        allJsonObjects.push(JSON.parse(nextDataMatch[1]));
      } catch (e) {}
    }
    ...
    // Combine all info for photo extraction
    ...
    // Extraction des métadonnées depuits tous les objets JSON trouvés
    const dataSource = allJsonObjects;

    // Helper functions to search in the pool of JSON objects
    ...
    res.status(500).json({ error: 'Failed to fetch the listing info.' });
  }
}
